...
const mockProjects: Project[] = [
  {
    ...
    description: 'A technology startup building software solutions for businesses',
    deck_count: 2,
...
export const FilterTest: React.FC = () => {
  ...
  const handleFilterChange = (filtered: Project[], filters: FilterOptions) => {
    console.log('Filter change triggered:', { filters, resultCount: filtered.length });
  ...
  return (
    <div className="max-w-6xl mx-auto p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-4">Filter Functionality Test</h1>
        <p className="text-gray-600">
          Test the ProjectFilters component to verify all filtering options work correctly.
        </p>
      </div>
      ...
      {currentFilters && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
          <h3 className="font-semibold text-blue-900 mb-2">Current Filters Applied:</h3>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
            ...
            <div>
              ...
              <span className="ml-2 text-blue-700">{currentFilters.industry}</span>
            </div>
            <div>
              <span className="font-medium text-blue-800">Date Range:</span>
              <span className="ml-2 text-blue-700">{currentFilters.dateRange}</span>
            </div>
            <div>
              <span className="font-medium text-blue-800">Deck Count:</span>
              <span className="ml-2 text-blue-700">{currentFilters.deckCount}</span>
            </div>
            <div>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6">
        <h3 className="font-semibold text-gray-900 mb-2">
          Filter Results: {filteredProjects.length} of {mockProjects.length} projects
        </h3>
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-gray-900">Filtered Projects:</h3>
        {filteredProjects.length === 0 ? (
            ...
            {filteredProjects.map((project) => (
              <div key={project.id} className="bg-white border border-gray-200 rounded-lg p-4">
                ...
                <div className="space-y-1 text-xs text-gray-500">
                  <div>
                    <span className="font-medium">Decks:</span> {project.deck_count}
                  </div>
                  <div>
      ...
      <div className="mt-8 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        ...
        <ul className="text-sm text-yellow-700 space-y-1">
          <li>• <strong>Search Test:</strong> Try searching for "tech", "health", or "payment"</li>
          <li>• <strong>Industry Test:</strong> Select "Technology", "Healthcare", or "Finance"</li>
          <li>• <strong>Date Test:</strong> Try "Past week", "Past month" filters</li>
          <li>• <strong>Deck Count Test:</strong> Filter by "No decks", "Few decks", "Many decks"</li>
          <li>• <strong>Sort Test:</strong> Change sort order and direction</li>
          <li>• <strong>Combined Test:</strong> Use multiple filters together</li>
        </ul>
      </div>
    </div>
  );
